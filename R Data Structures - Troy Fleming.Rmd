...
```{r}
# Vectors MUST BE THE SAME DATA TYPE
...
```{r}
# Returns the type of vector (characters, integers, etc)
typeof(names)
```
...
```{r}
# Returns the number of indexes in vector
length(names)
```
...
# Similar to python in terms of format
names[1]
```
...
```{r}
my_vector <- c(10,19,4,-2,-6,0,12000)
sort(my_vector)
```
...
```{r}
# Addes matching indexes of vectors
...
```{r}
# Subtracts matching indexes of vectors
...
```{r}
# Multiplies matching indexes of vectors
mult_vectors <- vector_1 * vector_2
mult_vectors
```
...
```{r}
# Divides matching indexes of vectors
div_vectors <- vector_1 / vector_2
div_vectors
```
...
```{r}
# In this example, it takes the first index and continues the operation where vector is missing corresponding indexes
# IE. It's adding 1 to 7, 2 to 8, and 3 to 9
vector_1 <- c(1,2,3)
vector_2 <- c(4,5,6,7,8,9)
...
my_matrix <- matrix(
  ...
  ncol <- 5,
  byrow = FALSE,
  dimnames <- list(c('ROW 1', 'ROW 2'), c('COL 1', 'COL 2', 'COL 3', 'COL 4',
                                          'COL 5'))
)
...
my_array <- array(
  ...
  dimnames <- list(c('ROW 1', 'ROW 2', 'ROW 3'), 
                   c('COL 1', 'COL 2', 'LUNCH'),
                   c('MATRIX', 'MATRIX RELOADED', 'MATRIX REVOLUTIONS'))
)
...
You can also use the optional levels parameter to specify the order in which the levels should be displayed. Must call using sort() function.
...
hw_array <- array(
  c(c(1, 2, 3, 4, 5, 6, 7, 8 ,9),c(9, 8, 7, 6, 5, 4, 3, 2, 1)),
  dim <- c(3, 3, 2)
)
